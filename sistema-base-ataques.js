/*==========================================================
=  SISTEMA BASE V2 — ATAQUES RÁPIDOS E CURA
==========================================================*/
(function(){
"use strict";
...
function findSavedEntry(character,index){
    const lists=[character?.quickAttacks,character?.attacks,character?.ataquesRapidos];
...
function updateAutomaticFormula(index){
    ...
    rollInput.dataset.criticalOn="12";
...
function configureAttackCard(index,character){
    ...
    if(index<=3){
        ...
        const skillValue=saved.skill||saved.testSkill||saved.pericia||rollInput.dataset.skill||"luta";
        ...
        if(!damageType){
            const savedType=normalizeText(saved.damageType||saved.tipoDano||"impacto").replace(/ç/g,"c")||"impacto";
            damageType=createSelect(
            ...
            const anchor=damageField.nextElementSibling;
            const field=document.createElement("div");
            ...
            damageField.parentElement.insertBefore(field,anchor);
        }
        ...
        const oldType=document.getElementById("attack4DamageType")?.closest(".field");
        oldType?.remove();
    }
...
function collectQuickEntries(){
    return [1,2,3,4].map(index=>{
        ...
        const entry={
            ...
            skill,
            skillName:formula.skillName,
            attribute:formula.attribute,
            roll:formula.text,
            rollBase:"1d12",
            training:formula.training,
            bonus:formula.bonus,
            penalty:formula.penalty,
            criticalOn:12,
            criticalDie:"main",
            criticalEffect:"roll-training-again"
        };
...
function patchCharacterMigration(){
    ...
    if(original.__systemV2AttacksPatched) return;
...
function updateAllFormulas(){
    [1,2,3,4].forEach(updateAutomaticFormula);
}
