...
function processEnemyRoundAbilities(){const round=enemyCombatRound();(currentTableCampaign?.enemies||[]).forEach(enemy=>{if(!enemyHasAbility(enemy,"regenerativo"))return;const state=enemyAbilityState(enemy);if(state.regenerativoRound===round)return;state.regenerativoRound=round;healEnemy(enemy,Math.max(0,Number(enemy.corpo)||0),"Regenerativo");});}
...
function useNewEnemyAbility(enemy,id,position){if(!["atrair","ilusorio"].includes(id))return false;refreshCurrentTableCampaign();const live=(currentTableCampaign.enemies||[]).find(item=>String(item.enemyId||item.id)===String(enemy.enemyId||enemy.id))||enemy;if(!enemyAbilityAvailability(live,id).enabled)return true;startEnemyAbilityTargetSelection(live,id,position);return true;}
...
function addRollChatMessage(
    ...
    if(currentTableRole==="player"&&metadata.rollKind==="attack"&&currentTableCharacter?.id){
        const penalties=currentTableCampaign.combat?.illusoryPenalties||{},effect=penalties[currentTableCharacter.id];
        if(effect){const penalty=Math.max(0,Number(effect.penalty)||0);total=(Number(total)||0)-penalty;formula=`${formula} - ${penalty}`;detail=`${detail}${detail?" • ":""}Ilusório: -${penalty}`;delete penalties[currentTableCharacter.id];saveTableCampaign();addSystemChatMessage(`${currentTableCharacter.name} sofreu -${penalty} no ataque por Ilusório.`);}
    }
...
function finishBodyDamageDistribution(){
    ...
    if(state.type==="player"){target.body.temporaryPV=Math.max(0,state.temporaryBefore-state.temporaryAbsorbed);saveDamagedCharacter(target);const sourceMessage=(currentTableCampaign.chatMessages||[]).find(item=>item.id===state.messageId);applyApplicatorCondition(target,sourceMessage?.enemyInstanceId);}else saveTableCampaign();
